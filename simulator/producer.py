...
def main() -> None:
    ...
    if not target_queues:
        logger.error(
            "No valid target queues discovered or configured for the Producer."
        )
        return

    run_token = _run_token()
    include_duplicate_case = (
        os.getenv("PRODUCER_INCLUDE_EXACT_DUPLICATE", "false").lower() == "true"
    )
    include_hard_crash_case = (
        os.getenv("PRODUCER_INCLUDE_HARD_CRASH", "false").lower() == "true"
    )
    ...
    test_scenarios: List[Dict[str, Any]] = [
        {
            "payload": {
                ...
                "currency": "USD",
                "account": f"1111-{run_token}",
            },
            "properties": {
                "client_id": f"Alpha_Corp_{run_token}",
        ...
        },
        {
            ...
            "properties": {
                "client_id": f"Beta_Corp_{run_token}",
                "message_type": "PaymentRequest",
                "Resubmit-Count": 3,
            },
        ...
        },
        {
            ...
            "properties": {
                "client_id": f"Gamma_Inc_{run_token}",
                "message_type": "PaymentRequest",
                "Resubmit-Count": 0,
            },
        ...
        },
        {
            "payload": {
                ...
                "force_circuit_breaker": True,
                "run_token": run_token,
            },
            "properties": {
                "client_id": f"Delta_LLC_{run_token}",
                "message_type": "PaymentRequest",
                "Resubmit-Count": 0,
            },
        ...
        },
        {
            "payload": {
                ...
                "client_status": "blacklisted",
                "run_token": run_token,
            },
            "properties": {
                "client_id": f"Epsilon_LLC_{run_token}",
        ...
        },
        {
            ...
            "properties": {
                "client_id": f"Kappa_Corp_{run_token}",
                "message_type": "PaymentRequest",
                "Resubmit-Count": 0,
            },
        ...
        },
        {
            "payload": {
                "currency": "GBP",
                "account": f"9999-{run_token}",
                "trigger_cache_test": True,
            },
            "properties": {
                "client_id": f"Gamma_Inc_{run_token}",
                "message_type": "PaymentRequest",
        ...
        },
        {
            ...
            "payload": {
                "plan": "premium",
                "trigger_string_inject": True,
                "run_token": run_token,
            },
            "properties": {
                "client_id": f"Theta_Corp_{run_token}",
        ...
        },
        {
            ...
            "properties": {
                "client_id": f"Omega_Corp_{run_token}",
                "message_type": "LegacySync",
                "Resubmit-Count": 0,
            },
    ...
    if include_hard_crash_case:
        test_scenarios.insert(
            6,
            {
                "payload": {
                    "transaction_amount": 10.00,
                    "trigger_hard_crash": True,
                    "run_token": run_token,
                },
                "properties": {
                    "client_id": f"Zeta_Corp_{run_token}",
                    "message_type": "PaymentRequest",
                    "Resubmit-Count": 0,
                },
    ...
    try:
        for queue_name in target_queues:
            with client.get_queue_sender(queue_name=queue_name) as sender:
                logger.info(
                    f"Producer connected to {queue_name}. Building synthetic batch..."
                )
                ...
                malformed_msg = ServiceBusMessage(
                    ...
                    application_properties={
                        "client_id": f"Sigma_Corp_{run_token}",
                        "message_type": "PaymentRequest",
                        "Resubmit-Count": 0,
                    },
                    correlation_id=str(uuid.uuid4()),
                ...
                if include_duplicate_case:
                    ...
                    logger.info(
                        "Skipping exact duplicate case unless PRODUCER_INCLUDE_EXACT_DUPLICATE=true"
                    )

                sender.send_messages(batch)
                logger.info(
                    f"Successfully dispatched {len(batch)} messages to {queue_name}."
                )
    finally:
        logger.info(
            "Producer dispatch complete. Closing shared AMQP connection pool..."
        )
        client.close()
