...
def process_queue(
    queue_name: str, fully_qualified_namespace: str, credential: DefaultAzureCredential
) -> None:
    ...
    try:
        with client.get_queue_receiver(
            ...
            while not shutdown_event.is_set():
                ...
                thread_logger.info(
                    f"Consumer processing {len(messages)} items from {queue_name}..."
                )
                ...
                for message in messages:
                    try:
                        ...
                        if (
                            ...
                            and b"message_type" in message.application_properties
                        ):
                            message_type = _to_text(
                                message.application_properties[b"message_type"]
                            )
                        elif (
                            message.application_properties
                            and "message_type" in message.application_properties
                        ):
                            message_type = _to_text(
                                message.application_properties["message_type"]
                            )
                        ...
                        try:
                            ...
                            thread_logger.error(
                                f"Rejecting {message.message_id}: Malformed JSON"
                            )
                        ...
                        if payload.get("mock_infra_001"):
                            receiver.dead_letter_message(
                                ...
                                error_description="TTL expired",
                            )
                            continue
                        if payload.get("mock_infra_003"):
                            receiver.dead_letter_message(
                                message,
                                reason="MaxTransferHopCountExceeded",
                                error_description="Routing loop",
                            )
                            continue
                        if payload.get("mock_infra_004"):
                            receiver.dead_letter_message(
                                message,
                                reason="MessageSizeExceeded",
                                error_description="Payload exceeds broker limits",
                            )
                            continue
                        ...
                        if (
                            ...
                            and "transaction_amount" not in payload
                        ):
                            receiver.dead_letter_message(
                                message,
                                reason="ValidationFailed",
                                error_description="missing mandatory field: 'transaction_amount'",
                            )
                            continue
                        # CRITICAL FIX: Target the customer_id field that exists in the safe_defaults_map
                        if (
                            message_type == "AccountSync"
                            and "customer_id" not in payload
                        ):
                            receiver.dead_letter_message(
                                message,
                                reason="ValidationFailed",
                                error_description="missing mandatory field: 'customer_id'",
                            )
                            continue
                        if payload.get("force_circuit_breaker"):
                            receiver.dead_letter_message(
                                message,
                                reason="CircuitBreakerOpenException",
                                error_description="Upstream API refused",
                            )
                            continue
                        if payload.get("client_status") == "blacklisted":
                            receiver.dead_letter_message(
                                ...
                                error_description="Blacklisted client",
                            )
                            continue
                        if payload.get("trigger_unknown_fault"):
                            receiver.dead_letter_message(
                                message,
                                reason="SystemFault",
                                error_description="Unexpected null pointer",
                            )
                            continue
                        ...
                        thread_logger.error(
                            f"Consumer hard-crashed on {message.message_id}: {e}"
                        )
                        try:
                            receiver.abandon_message(message)
                        except Exception as abandon_err:
                            thread_logger.error(
                                f"Failed to abandon message (Broker offline?): {abandon_err}"
                            )
                        continue
...
def main() -> None:
    ...
    if not target_queues:
        logger.error(
            "No valid target queues discovered or configured for the Consumer."
        )
        return

    is_local_emulator = "usedevelopmentemulator=true" in connection_string.lower()
    configured_workers = int(os.getenv("MAX_CONCURRENT_QUEUES", 5))
    max_workers = (
        min(configured_workers, 1) if is_local_emulator else configured_workers
    )
    if is_local_emulator and configured_workers > 1:
        logger.warning(
            "Capping simulator consumer concurrency at 1 for the local Service Bus "
            "emulator to avoid ConnectionsQuotaExceeded errors."
        )
    cycle_sleep = int(os.getenv("AGENT_CYCLE_SLEEP_SECONDS", 60))
    logger.info(
        f"Starting simulated consumers for {len(target_queues)} queues across {max_workers} threads."
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while not shutdown_event.is_set():
            futures = {
                executor.submit(
                    process_queue, q_name, fully_qualified_namespace, credential
                ): q_name
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.error(
                        f"Thread processing {q_name} generated an exception: {exc}"
                    )

            if not shutdown_event.is_set():
                logger.info(
                    f"Consumer cycle complete. Sleeping for {cycle_sleep} seconds to prevent AMQP churn."
                )
