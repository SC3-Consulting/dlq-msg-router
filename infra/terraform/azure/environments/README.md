...
- `platform.tfvars`: Checked into source control. Holds non-secret, environment-specific settings including immutable `router_container_image_tag` and `notification_container_image_tag`, `app_env_vars`, and `app_secret_env_var_secret_names` mappings.
