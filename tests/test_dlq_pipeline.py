...
def test_azure_foundry_engine_execution(mock_azure_client, monkeypatch):
    ...
    monkeypatch.setenv("AZURE_FOUNDRY_DEPLOYMENT_NAME", "gpt-5.1-chat")
...
def test_azure_foundry_engine_retries_on_rate_limit(
    ...
    cast(Any, azure_engine.client).complete.side_effect = [
        Exception(
            "(rate_limit_exceeded) Your requests to gpt-5.1-chat have exceeded rate limit."
        ),
        mock_response,
    ]
...
def test_azure_foundry_engine_normalizes_base_endpoint(mock_azure_client, monkeypatch):
    ...
    assert azure_engine.endpoint.endswith("/openai/deployments/gpt-5.1-chat")
